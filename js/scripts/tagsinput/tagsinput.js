(function ($) {
  ...
  TagsInput.prototype = {
    ...
    add: function(item, dontPushVal, options) {
      ...
      if (self.isSelect && !jQuery('option[value="' + encodeURIComponent(itemValue) + '"]',self.$element)[0]) {
      ...
      self.$container.on('click', $.proxy(function(event) {
        ...
        if (self.options.addOnBlur && self.options.freeInput) {
          self.$input.on('focusout', $.proxy(function(event) {
              ...
              if (jQuery('.typeahead, .twitter-typeahead', self.$container).length === 0) {
    ...
    this.each(function() {
      ...
      if (!tagsinput) {
          ...
          if (this.tagName === 'SELECT') {
              jQuery('option', jQuery(this)).attr('selected', 'selected');
          }
